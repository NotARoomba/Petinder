...
export default function Centers() {
  ...
  const [show, setShow] = useState<Center>();
  ...
  useEffect(() => {
    ...
    async function updateData() {
      const { centers } = await callAPI("/centers/", "GET", {});
      setData(centers);
    }
    updateData();
    updateTheme();
  }, []);
  const showCenter = (_k: never, childProps: Center) => {
    setShow(childProps);
  };
  return (
    <div className="min-h-[calc(100vh-208px)] dark:bg-gunmetal-300 dark:text-gunmetal-900 text-gunmetal pt-2 mb-auto ">
      ...
      <div className="w-10/12 h-[55vh] xl:h-[60vh] justify-center mx-auto">
        {theme && (
          <GoogleMapReact
            ...
            onChildClick={showCenter}
            onClick={() => setShow(undefined)}
            options={{ styles: theme == ThemeState.LIGHT ? [] : DarkTheme }}
          >
            {data.map((v, i) => (
              <Marker
                opacity={opacity}
