...
export default function Adopt() {
  ...
  useEffect(() => {
    ...
    async function updateData() {
      const { animals } = await callAPI("/animals/", "GET", {});
      setData(animals);
      setShowing(animals);
      setLoading(false);
    }
  ...
  const updateFilter = (
    filters: MultiValue<{ value: string; label: string }>,
  ) => {
    setShowing(data);
    for (const filter of filters) {
      setShowing(
        showing.filter(
          (v) =>
            v.color === filter.value ||
            v.animal === filter.value ||
            v.size === filter.value,
        ),
      );
    }
  ...
  return (
    <div className="dark:bg-gunmetal-300 dark:text-gunmetal-900 text-gunmetal min-h-[calc(100vh-208px)] pt-2 mb-auto">
      ...
      <Select
        isMulti
        styles={style}
        className="w-11/12 justify-center m-auto rounded-full border-8 dark:border-gunmetal-500 border-white shadow-2xl "
        ...
        options={[
          ...new Set(data.map((v) => v.animal)),
          ...new Set(data.map((v) => v.color)),
          ...new Set(data.map((v) => v.size)),
        ].map((v) => ({ value: v, label: v }))}
        onChange={(newValue) => updateFilter(newValue)}
      />
      {/* need to divide the array in 4 and then distribute among the columns */}
      {showing.length != 0 ? (
        <div className=" md:columns-1 lg:columns-2 xl:columns-3 2xl:columns-4 gap-x-24 gap-y-4 text-center w-full overflow-y-auto py-10 justify-items-center mx-auto">
          {showing.map((v, i) => (
            <Card
              name={v.name}
              description={v.animal}
              image={v.image}
              key={i}
            />
          ))}
        ...
        <p className="text-center text-4xl text-rose-500 mt-12 font-bold">
          No encontramos ningun animal con esos tags!
        </p>
      ) : (
        <p className="text-center text-4xl mt-12 font-bold">Cargando...</p>
      )}
    </div>
  );
}
